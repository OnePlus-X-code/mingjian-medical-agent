...
```text
DRG 疑似清单 → Agent 智能复核 → 匹配历史申诉案例 → 生成红黄绿分级结果
...
点击任一案例，查看 DRG 原始疑点、Agent 判断理由、相似历史申诉案例（Top 3）、相似度评分、病历关键点摘要、证据链时间线、建议操作。

### 🤖 Agent 智能复核

- 调用 LLM 大模型（DeepSeek / 通义千问 / 其他 OpenAI 兼容 API）分析病例
- 匹配历史申诉案例、读取医院科室画像、生成结构化复核建议
- 支持规则兜底模式：LLM 不可用时自动回退，保证系统稳定
...
│   └── requirements.txt
├── data/                        # 业务数据
│   ├── suspect_cases.json       # DRG 疑似违规清单（后台输入）
│   ├── appeal_cases.json        # 历史申诉案例库
│   ├── hospital_profiles.json   # 医院科室画像
